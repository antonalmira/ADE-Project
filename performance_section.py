...
class PerformanceSection:
    ...
    def _process_node(self, node, doc, last_element):
        ...
        if not is_root and node.data(0, Qt.UserRole + 2) == "folder":
            clean_name = self._strip_chapter_numbering(
                node.text(0)
    ...
    def _render_file_item(self, node, doc, last_element):
        ...
        if item_type == "table":
            if excel_path and os.path.exists(excel_path):
                ...
                tb_data = None
                if voltage and voltage in split_tables:
                    tb_data = split_tables[voltage]
                elif "" in split_tables:
                    tb_data = split_tables[""]
            ...
            c_l = int(crop_data.get('left',   0))
            c_t = int(crop_data.get('top',    0))
            c_r = int(crop_data.get('right',  0))
            c_b = int(crop_data.get('bottom', 0))
        ...
        if isinstance(custom_cap, dict):
            for key in ['ch_info', 'zoom_info', 'meas_info']:
                ...
                if text_val:
                    ...
                    last_element.getparent().insert(
                        last_element.getparent().index(last_element) + 1, p._element
                    )
    ...
    def _format_data(self, data):
        """Convert raw cell values to display strings, preserving enough precision for engineering use."""
        formatted = []
        for row in data:
            new_row = []
            for cell in row:
                if isinstance(cell, float):
                    if cell == 0.0:
                        ...
                        try:
                            # Use 6 significant figures to avoid silently rounding
                            # engineering values such as efficiency percentages.
                            val_str = f"{cell:.6g}"
