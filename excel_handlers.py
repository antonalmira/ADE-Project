...
def extract_chart_screenshots(app, excel, performance_items, performance_folder, charts_base_dir):
    ...
    for item_name, files in selected_files.items():
        ...
        for file_name in files:
            ...
            try:
                ...
                if not chart_sheets:
                    # No charts found — export the first table as a formatted screenshot
                    ...
                    try:
                        ...
                        excel_window = next(
                            (w for w in windows if "Microsoft Excel" in w.title), None
                        )
                        if excel_window and excel_window.width > 0 and excel_window.height > 0:
                            screenshot = pyautogui.screenshot(
                                region=(excel_window.left, excel_window.top,
                                        excel_window.width, excel_window.height)
                            )
                ...
                for sheet_name in chart_sheets:
                    ...
                    if not success:
                        print(f"All methods failed for {sheet_name}.")
                        failure_count += 1
    ...
    elif failure_count < total_files:
        show_popup(
            app, "Partial Success",
            f"Charts extracted with {failure_count} failure(s). Check the console for details.",
            "warning"
        )
    else:
        show_popup(app, "Error", "All chart exports failed. Check the console for details.", "error")

    return failure_count < total_files